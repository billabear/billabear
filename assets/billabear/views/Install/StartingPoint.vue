  ...
  <div class="w-screen">
    ...
    <div class="mx-auto login max-w-2xl mt-12 mb-12 border rounded-lg bg-white shadow p-5">
      <form @submit.prevent="handleSubmit" v-if="!complete">
        <div class="mx-auto" :class="{'animate-shake': error_info.has_error}">
          <div class="w-full">
            <PublicLogo />
          </div>
          <h1 class="h1 text-center text-3xl">{{ $t('install.title') }}</h1>
          <div class="px-5 mt-2 mb-3" v-if="errorMessage">
            <div class="alert-error text-center">{{ errorMessage }}</div>
          </div>

          <div class="text-center text-xl mt-5 mb-3">
            <h2>{{  $t('install.settings.title') }}</h2>
          </div>
          <div class="px-5 mb-3">
            <label class="block mb-1">{{ $t('install.settings.default_brand') }}</label>
            <p class="form-field w-full-error" v-if="errors.defaultBrand != undefined">{{ errors.defaultBrand }}</p>
            <input type="text" class="form-field w-full" v-model="default_brand" />
          </div>
          <div class="px-5 mb-3">
            <label class="block mb-1">{{ $t('install.settings.country') }}</label>
            <p class="form-field w-full-error" v-if="errors.country != undefined">{{ errors.country }}</p>
            <CountrySelect class="w-full" v-model="country" />
          </div>
          <div class="px-5 mb-3">
            <label class="block mb-1">{{ $t('install.settings.currency') }}</label>
            <p class="form-field w-full-error" v-if="errors.currency != undefined">{{ errors.currency }}</p>
            <CurrencyRate class="w-full"  v-model="currency" />
          </div>
          <div class="px-5 mb-3">
            <label class="block mb-1">{{ $t('install.settings.from_email') }}</label>
            <p class="form-field w-full-error" v-if="errors.fromEmail != undefined">{{ errors.fromEmail }}</p>
            <input type="text" class="form-field w-full" v-model="from_email" />
          </div>
          <div class="px-5 mb-3">
            <label class="block mb-1">{{ $t('install.settings.timezone') }}</label>
            <p class="form-field w-full-error" v-if="errors.timezone != undefined">{{ errors.timezone }}</p>
            <select class="form-field w-full" v-model="timezone">
          ...
          <div class="px-5 mb-3">
            ...
            <p class="form-field w-full-error" v-if="errors.webhooKurl != undefined">{{ errors.webhooKurl }}</p>
            <input type="text" class="form-field w-full" v-model="webhook_url" />
          </div>
          ...
          <div class="px-5 mb-3">
            ...
            <p class="form-field w-full-error" v-if="errors.email != undefined">{{ errors.email }}</p>
            <input type="text" class="form-field w-full" v-model="email" />
          </div>
          <div class="px-5 mb-3">
            <label class="block mb-1">{{ $t('install.user.password') }}</label>
            <p class="form-field w-full-error" v-if="errors.password != undefined">{{ errors.password }}</p>
            <input type="password" class="form-field w-full" v-model="password" />
          </div>
          <div class="px-5">
            <SubmitButton class="w-full" :in-progress="sendingInProgress">{{ $t('install.submit_button') }}</SubmitButton>
          </div>
        </div>
      </form>
      <div v-else>
