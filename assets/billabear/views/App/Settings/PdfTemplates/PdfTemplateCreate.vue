<template>
  ...
  <div>
    <LoadingScreen :ready="ready">
      <h1 class="page-title">{{ $t('app.settings.pdf_template.create.title') }}</h1>
      ...
      <div class="card-body m-5">
        <div class="form-field-ctn">
          ...
          <input type="text" class="form-field" id="locale" v-model="template.locale"  />
          <p class="form-field-help">{{ $t('app.settings.pdf_template.create.help_info.locale') }}</p>
          ...
          <p class="form-field-help">{{ $t('app.settings.pdf_template.create.help_info.type') }}</p>
          ...
          <select v-model="template.brand" class="form-field">
            <option v-for="(brand, code) in brands" :value="brand.code">{{ brand.name }}</option>
          </select>
          <p class="form-field-help">{{ $t('app.settings.pdf_template.create.help_info.brand') }}</p>
          ...
          <textarea class="form-field" rows="10" cols="80" v-model="template.template"></textarea>
          <p class="form-field-help">{{ $t('app.settings.pdf_template.create.help_info.template') }}</p>
...
export default {
  ...
  mounted() {
    ...
    this.ready=true;
    axios.get('/app/settings/template/create').then(response => {
      this.ready = true;
  ...
  methods: {
    save: function () {
      ...
      axios.post('/app/settings/template/create', this.template).then(response => {
