  ...
  <div>
    ...
    <LoadingScreen :ready="ready">
      <div class="card-body">
        ...
        <div class="alert-error mb-3" v-if="complete_error">{{ $t('app.integrations.crm.errors.complete_error') }}</div>
        ...
        <div class="form-field-ctn mt-3" v-if="integration!== null && integration.authentication_type == 'oauth'">
          <a :href="'/app/'+integration.name+'/oauth/start'" class="btn--main" v-if="enabled == false">{{ $t('app.integrations.crm.buttons.connect') }}</a>
          <button class="btn--main" @click="disconnectOauth()" v-else>{{ $t('app.integrations.crm.buttons.disconnect') }}</button>
        </div>
      </div>

      <div class="card-body mt-3" v-if="integration !== null && integration.settings.length > 0">
        <h2 class="text-2xl">{{ $t('app.integrations.crm.settings.title') }}</h2>
        <div class="form-field-ctn" >
          <label class="form-field-lbl" for="name">
      ...
      <SubmitButton :in-progress="send_request" class="btn--main mt-3" @click="saveSettings()">{{ $t('app.integrations.crm.buttons.save') }}</SubmitButton>
...
export default {
  ...
  methods: {
    ...
    saveSettings() {
      ...
      axios.post('/app/integrations/crm/settings', {enabled: this.enabled, integration_name: this.integration.name, settings: settings}).then(response => {
        this.send_request = false;
      }).catch(error => {
