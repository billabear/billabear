      ...
      <div class="flex">
        ...
        <div class="pl-5 flex-1">
          ...
          <div class="rounded-lg bg-white shadow p-3">
            ...
            <th class="text-left pb-2">{{ $t('app.workflows.refund_created_process.list.email') }}</th>
            <th class="text-left pb-2">{{ $t('app.workflows.refund_created_process.list.status') }}</th>
            <th></th>
          </tr>
          </thead>
          <tbody v-if="loaded">
          <tr v-for="subscription in subscriptions" class="mt-5">
            <td class="py-3">{{ subscription.refund.customer.email }}</td>

            <td class="py-3">{{ subscription.state }}</td>
            <td class="py-3"><router-link :to="{name: 'app.workflows.refund_created_process.view', params: {id: subscription.id}}" class="btn--main">{{ $t('app.workflows.refund_created_process.list.view') }}</router-link></td>
          </tr>
          <tr v-if="subscriptions.length === 0">
            <td colspan="4" class="py-3 text-center">{{ $t('app.workflows.refund_created_process.list.no_results') }}</td>
          </tr>
          </tbody>
          <tbody v-else>
...
export default {
  ...
  mounted() {
    ...
    this.loadRefundCreatedProcesses();
  ...
  methods: {
    ...
    loadRefundCreatedProcesses: function ()
    {
