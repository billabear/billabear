<template>
  <div>
    <h1 class="page-title">{{ $t('app.workflows.refund_created_process.list.title') }}</h1>
    ...
    <div class="card-body m-5" v-if="active_filters.length > 0">
      ...
      <button @click="doSearch" class="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">{{ $t('app.workflows.refund_created_process.list.filter.search') }}</button>
    </div>
    ...
    <LoadingScreen :ready="ready">
      <div>
        <table class="list-table">
          <thead>
          <tr>
            <th>{{ $t('app.workflows.refund_created_process.list.email') }}</th>
            <th>{{ $t('app.workflows.refund_created_process.list.status') }}</th>
            <th></th>
          </tr>
          </thead>
          <tbody v-if="loaded">
          <tr v-for="subscription in subscriptions" class="mt-5">
            <td>{{ subscription.refund.customer.email }}</td>

            <td>{{ subscription.state }}</td>
            <td><router-link :to="{name: 'app.workflows.refund_created_process.view', params: {id: subscription.id}}" class="btn--main">{{ $t('app.workflows.refund_created_process.list.view') }}</router-link></td>
          </tr>
          <tr v-if="subscriptions.length === 0">
            <td colspan="4" class="text-center">{{ $t('app.workflows.refund_created_process.list.no_results') }}</td>
          </tr>
          </tbody>
          <tbody v-else>
          <tr>
            <td colspan="4" class="text-center">
              <LoadingMessage>{{ $t('app.workflows.refund_created_process.list.loading') }}</LoadingMessage>
            </td>
          </tr>
          </tbody>
        </table>
      ...
      <div class="sm:grid sm:grid-cols-2">
        ...
        <div class="m-5">
          <button @click="prevPage" v-if="show_back" class="btn--main mr-3" >{{ $t('app.workflows.refund_created_process.list.prev') }}</button>
          <button @click="nextPage" v-if="has_more" class="btn--main" >{{ $t('app.workflows.refund_created_process.list.next') }}</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    doStuff: function ()
    {
      ...
      var mode = 'normal';
      let urlString = '/app/system/refund-created-process/list?';
