<template>
  <div v-if="!has_error">
    <h1 class="ml-5 mt-5 page-title">{{ $t('app.payment.list.title') }}</h1>
        ...
        <div class="pl-5 flex-1">
          ...
          <thead>
            <tr class="border-b border-black">
              <th class="text-left pb-2">{{ $t('app.payment.list.list.amount') }}</th>
              <th class="text-left pb-2">{{ $t('app.payment.list.list.currency')}}</th>
              <th class="text-left pb-2">{{ $t('app.payment.list.list.customer') }}</th>
              <th class="text-left pb-2">{{ $t('app.payment.list.list.created_at') }}</th>
              <th></th>
            </tr>
          </thead>
          <tbody v-if="loaded">
            <tr v-for="payment in payments" class="mt-5 cursor-pointer">
              <td class="py-3">{{ currency(payment.amount) }}</td>
              <td class="py-3">{{ payment.currency }}</td>
