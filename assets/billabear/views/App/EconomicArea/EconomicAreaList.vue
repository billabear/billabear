<template>
  <div v-if="!has_error">
    <h1 class="ml-5 mt-5 page-title">{{ $t('app.economic_area.list.title') }}</h1>
    ...
    <div class="top-button-container">
      <div class="list">
        ...
        <RoleOnlyView role="ROLE_ACCOUNT_MANAGER">
          <router-link :to="{name: 'app.finance.economic_area.create'}" class="btn--main ml-4"><i class="fa-solid fa-plus"></i> {{ $t('app.economic_area.list.create_new') }}</router-link>
        </RoleOnlyView>
      </div>
    </div>
    ...
    <div class="mt-3">
        <table class="list-table">
          <thead>
            <tr>
              <th>{{ $t('app.economic_area.list.list.name') }}</th>
              <th>{{ $t('app.economic_area.list.list.tax_threshold')}}</th>
              <th></th>
            </tr>
          </thead>
          <tbody v-if="loaded">
            <tr v-for="area in areas" class="mt-5 cursor-pointer">
              <td>{{ area.name }}</td>
              <td><Currency :amount="area.threshold" :currency="area.currency" /></td>
              <td><router-link :to="{name: 'app.finance.economic_area.view', params: {id: area.id}}" class="list-btn">{{ $t('app.economic_area.list.view') }}</router-link></td>
            </tr>
            <tr v-if="areas.length === 0">
              <td colspan="5" class="text-center">{{ $t('app.economic_area.list.no_countries') }}</td>
            </tr>
          </tbody>
          <tbody v-else>
            <tr>
              <td colspan="5" class="text-center">
                <LoadingMessage>{{ $t('app.economic_area.list.loading') }}</LoadingMessage>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="sm:grid sm:grid-cols-2">
        ...
        <div class="mt-4">
          <button @click="prevPage" v-if="show_back" class="btn--main mr-3" >{{ $t('app.economic_area.list.prev') }}</button>
          <button @click="nextPage" v-if="has_more" class="btn--main" >{{ $t('app.economic_area.list.next') }}</button>
        </div>
...
export default  {
  name: "EconomicAreaList.vue",
  components: {Currency, ListGroupItem, ListGroup, Dropdown, InternalApp, RoleOnlyView},
  data() {
    return {
      ready: false,
      areas: [],
  ...
  methods: {
    ...
    doStuff: function ()
    {
      ...
      var mode = 'normal';
      let urlString = '/app/economic-areas?';
      if (this.$route.query.last_key !== undefined) {
