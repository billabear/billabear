        ...
        <form @submit.prevent="handleSubmit" v-if="!successfully_progress">
          ...
          >
            ...
            <h1 class="h1 text-center">
              {{ $t('public.forgot_password_confirm.title') }}
            </h1>
            ...
            <div class="px-5 mb-3">
              <label class="block mb-1">{{
                $t('public.forgot_password_confirm.password')
              }}</label>
              <input type="password" class="input-field" v-model="password" />
              <span
                class="block text-red-500"
                v-if="password_error !== undefined"
                >{{ password_error }}</span
              >
            </div>
            <div class="px-5 mb-3">
              <label class="block mb-1">{{
                $t('public.forgot_password_confirm.password_confirm')
              }}</label>
              <input
                type="password"
                class="input-field"
              ...
              <span
                class="block text-red-500"
                v-if="password_confirm_error !== undefined"
                >{{ password_confirm_error }}</span
              >
            </div>
            <div class="px-5">
              <button
                ...
                v-if="!in_progress"
              >
                {{ $t('public.forgot_password_confirm.reset_button') }}
              </button>
              <button
                type="submit"
          ...
          <div class="p-5 public-form-body">
            ...
            <div class="px-5 pt-2 text-center">
              <router-link
                ...
                >{{
                  $t('public.forgot_password_confirm.login_link')
                }}</router-link
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    handleSubmit: function () {
      ...
      if (this.password === '') {
        this.password_error = this.$t(
          'public.forgot_password_confirm.password_error'
        )
        hasError = true
      } else if (this.password.length < 8) {
        this.password_error = this.$t(
          'public.forgot_password_confirm.password_length_error'
        )
        hasError = true
      } else {
        hasPassword = true
      ...
      if (this.password !== this.password_confirm) {
        this.password_confirm_error = this.$t(
          'public.forgot_password_confirm.password_confirm_error'
        )
        hasError = true
      }
