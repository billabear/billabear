      ...
      <form @submit.prevent="handleSubmit" v-if="signing_up">
        ...
        >
          ...
          <div class="px-5 mb-3">
            ...
            <span class="block text-red-500" v-if="email_error !== undefined">{{
              email_error
            }}</span>
          </div>
          <div class="px-5 mb-3">
          ...
          <div class="px-5 mb-3">
            <label class="block mb-1">{{
              $t('public.signup.password_confirm')
            }}</label>
            <input
              type="password"
              class="input-field"
          ...
          <div class="px-5 pt-3">
            <SubmitButton
              ...
              :loading-text="$t('public.signup.signing_up')"
            >
          ...
          <div class="mt-5 px-5 mb-3 text-center">
            <router-link
              ...
              >{{ $t('public.signup.login_link') }}</router-link
            >
          </div>
        </div>
      </form>
    ...
    handleSubmit(e) {
      ...
      if (password === '') {
        ...
        this.password_confirm_error = this.$t(
          'public.signup.password_confirm_error'
        )
        hasError = true
      }
      ...
      const that = this
      if (email && password) {
        userservice.signup(user, this.$route.params.code).then(
          (response) => {
