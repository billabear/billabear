<template>
  <div>
    <h1 class="ml-5 mt-5 page-title">
      {{ $t('app.subscription.create.title') }}
    </h1>
      ...
      <div>
        <div class="mt-3 card-body">
          ...
          <div>
            <select
              class="form-field"
              v-model="subscription_plan"
              @change="refreshTrial"
            >
              <option :value="null"></option>
              <option
                v-for="subscriptionPlan in subscription_plans"
        ...
        <div class="mt-3 card-body">
          ...
          <div>
            <select
              ...
              v-model="price"
              :disabled="
                subscription_plan == null || eligiblePrices.length == 0
              "
            >
              <option v-for="price in eligiblePrices" :value="price.id">
                {{ price.display_value }} - {{ price.schedule }}
              </option>
              <option v-if="subscription_plan == null" selected></option>
              <option v-else-if="eligiblePrices.length == 0" selected>
                {{ $t('app.subscription.create.no_eligible_prices') }}
              </option>
            </select>
          </div>
          <p class="form-field-help" v-if="eligible_currency != null">
            {{ $t('app.subscription.create.help_info.eligible_prices') }}
          </p>
        </div>

        <div
          class="mt-3 card-body"
          v-if="subscription_plan != null && subscription_plan.per_seat == true"
        >
          ...
          <div>
            <p
              ...
              v-if="errors['seatNumber'] != undefined"
            >
              {{ errors['seatNumber'] }}
            </p>
          ...
          <p class="form-field-help" v-if="eligible_currency != null">
            {{ $t('app.subscription.create.help_info.seats') }}
          </p>
        </div>
        ...
        <div class="mt-3 card-body" v-if="customer.billing_type != 'invoice'">
          ...
          <div>
            <select class="form-field" v-model="payment_detail">
              <option
                v-for="paymentDetail in payment_details"
        ...
        <div class="mt-3 card-body">
          ...
          <div class="form-field-ctn">
            ...
            <p class="form-field-error" v-if="errors['trial'] != undefined">
              {{ errors['trial'] }}
            </p>
            <input
              type="checkbox"
              v-model="trial"
              :disabled="eligible_currency != null"
            />
            <p class="form-field-help" v-if="eligible_currency != null">
              {{ $t('app.subscription.create.help_info.trial') }}
            </p>
            <p
              class="form-field-help"
              v-if="subscription_plan != null && !subscription_plan.has_trial"
            >
              {{ $t('app.subscription.create.help_info.no_trial') }}
            </p>
          </div>
          <div class="form-field-ctn">
            <label class="form-field-lbl" for="country">
            ...
            <input
              type="number"
              class="form-field"
              v-model="trial_length_days"
              :disabled="eligible_currency != null || !trial"
            />
          </div>
        </div>
        <div class="form-field-submit-ctn mt-5">
          <button
            ...
            class="btn--main"
            disabled
            v-if="price == null || subscription_plan == null"
          >
            {{ $t('app.subscription.create.submit_btn') }}
          </button>
          <SubmitButton :in-progress="sendingInProgress" v-else>{{
            $t('app.subscription.create.submit_btn')
          }}</SubmitButton>
        </div>
        <p class="text-green-500 font-weight-bold" v-if="success">
          {{ $t('app.subscription.create.success_message') }}
        </p>
      </div>
    </form>
  </div>
...
export default {
  ...
  computed: {
    eligiblePrices() {
      ...
      for (let i = 0; i < this.subscription_plan.prices.length; i++) {
        if (
          this.subscription_plan.prices[i].currency == this.eligible_currency &&
          this.subscription_plan.prices[i].schedule == this.eligible_schedule
        ) {
  ...
  mounted() {
    ...
    this.id = customerId
    axios
      .get('/app/customer/' + customerId + '/subscription')
      .then((response) => {
  ...
  methods: {
    ...
    send: function () {
      if (this.price === null || this.subscription_plan === null) {
        return
      }
      ...
      axios
        .post('/app/customer/' + customerId + '/subscription', payload)
        .then((response) => {
          ...
          this.$router.push({
            ...
            params: { subscriptionId: response.data.id },
          })
        })
        .catch((error) => {
