<template>
  <div>
    <h1 class="mt-5 ml-5 page-title">
      {{ $t('app.subscription.mass_change.create.title') }}
    </h1>
    ...
    <LoadingScreen :ready="ready">
      <div class="">
        <div class="mt-3 card-body">
          <h2>
            {{ $t('app.subscription.mass_change.create.criteria.title') }}
          </h2>
          ...
          <div class="mt-5">
            ...
            <p
              ...
              v-if="errors['targetPlan'] != undefined"
            >
              {{ errors['targetPlan'] }}
            </p>
            <select class="form-field" v-model="payload.target_plan" @change="">
              ...
              <option
                v-for="subscriptionPlan in plans"
          ...
          <div class="mt-5">
            ...
            <p
              class="form-field-error"
              v-if="errors['targetPrice'] != undefined"
            >
              {{ errors['targetPrice'] }}
            </p>
            <select
              class="form-field"
              v-model="payload.target_price"
              @change=""
            >
              <option :value="null"></option>
              <option v-for="price in targetPrices" :value="price">
          ...
          <div class="mt-5">
            ...
            <p
              class="form-field-error"
              v-if="errors['targetBrand'] != undefined"
            >
              {{ errors['targetBrand'] }}
            </p>
            <select
              class="form-field"
              v-model="payload.target_brand"
              @change=""
            >
              <option :value="null"></option>
          ...
          <div class="mt-5">
            ...
            <p
              class="form-field-error"
              v-if="errors['targetCountry'] != undefined"
            >
              {{ errors['targetCountry'] }}
            </p>
            <CountrySelect v-model:value="payload.target_country" />
          </div>
        </div>
        <div class="mt-3 card-body">
          ...
          <div class="mt-5">
            ...
            <p class="form-field-error" v-if="errors['newPlan'] != undefined">
              {{ errors['newPlan'] }}
            </p>
            <select
              class="form-field"
              v-model="payload.new_plan"
              @change="fetchEstimate"
            >
              <option :value="null"></option>
              <option
                v-for="subscriptionPlan in newPlans"
                :value="subscriptionPlan"
              >
                {{ subscriptionPlan.product.name }} -
                {{ subscriptionPlan.name }}
          ...
          <div class="mt-5">
            ...
            <p class="form-field-error" v-if="errors['newPrice'] != undefined">
              {{ errors['newPrice'] }}
            </p>
            <select class="form-field" v-model="payload.new_price" @change="">
              <option :value="null"></option>
              <option v-for="price in newPrices" :value="price">
                {{ price.display_value }} / {{ price.schedule }}
              </option>
            </select>
          </div>
        ...
        <div class="mt-3 card-body">
          <h2>
            {{ $t('app.subscription.mass_change.create.change_date.title') }}
          </h2>
          <p>
            {{
              $t('app.subscription.mass_change.create.change_date.help_info')
            }}
          </p>

          <p class="form-field-error" v-if="errors['changeDate'] != undefined">
            {{ errors['changeDate'] }}
          </p>
          <VueDatePicker
            class="mt-2"
            v-model:value="payload.change_date"
        ...
        <div class="mt-5 card-body" v-if="estimate !== null">
          {{
            $t('app.subscription.mass_change.create.estimate.amount', {
              amount: currency(this.estimate.amount),
              currency: this.estimate.currency,
              schedule: this.estimate.schedule,
            })
          }}
        </div>

        <div class="mt-5">
          <SubmitButton :in-progress="sending" @click="sendCreate">{{
            $t('app.subscription.mass_change.create.submit_button')
          }}</SubmitButton>
        </div>
      </div>
    </LoadingScreen>
...
export default {
  ...
  computed: {
    ...
    newPrices: function () {
      ...
      if (
        this.payload.target_price !== null &&
        this.payload.target_plan == null
      ) {
        const prices = []
        const len = this.prices.length
        for (let i = 0; i < len; i++) {
          if (
            this.prices[i].product.id == this.payload.target_price.product.id
          ) {
            prices.push(this.prices[i])
          }
        }
  ...
  methods: {
    ...
    sendEstimate: function () {
      ...
      this.errors = {}
      axios
        .post('/app/subscription/mass-change/estimate', payload)
        .then((response) => {
          this.sending = false
    ...
    sendCreate: function () {
      ...
      axios
        .post('/app/subscription/mass-change', payload)
        .then((response) => {
          this.sending = false
          this.$router.push({
