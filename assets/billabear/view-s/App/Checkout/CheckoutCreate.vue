      ...
      <div class="">
        <div class="card-body">
          <div class="form-field-ctn">
            ...
            <p class="form-field-help">
              {{ $t('app.checkout.create.customer.help_info.name') }}
            </p>
          </div>
          ...
          <div class="form-field-ctn">
            ...
            <p class="form-field-help">
              {{ $t('app.checkout.create.customer.help_info.brand') }}
            </p>
          </div>
          ...
          <div class="form-field-ctn">
            ...
            <input
              type="checkbox"
              class="form-field"
              v-model="checkout.permanent"
            />
            <p class="form-field-help">
              {{ $t('app.checkout.create.customer.help_info.permanent') }}
            </p>
          </div>
          <div class="form-field-ctn" v-if="checkout.permanent">
            ...
            <p class="form-field-error" v-if="errors.slug != undefined">
              {{ errors.slug }}
            </p>
            <input type="text" class="form-field" v-model="checkout.slug" />
            <p class="form-field-help">
              {{ $t('app.checkout.create.customer.help_info.slug') }}
            </p>
          </div>
          <div class="form-field-ctn">
            ...
            <p class="form-field-error" v-if="errors.customer != undefined">
              {{ errors.customer }}
            </p>
            <Autocomplete
              display-key="email"
              ...
              v-model:value="checkout.customer"
              :blur-callback="blurCallback"
            />
            <SubmitButton
              :in-progress="send_create_customer"
              @click="createCustomer"
              v-if="create_customer"
              >{{
                $t('app.checkout.create.customer.create_customer')
              }}</SubmitButton
            >
            <p class="form-field-help">
              {{ $t('app.checkout.create.customer.help_info.customer') }}
            </p>
          </div>
          ...
          <div class="form-field-ctn">
            ...
            <CurrencySelect v-model:value="checkout.currency" />
            <p class="form-field-help">
              {{ $t('app.checkout.create.customer.help_info.currency') }}
            </p>
          </div>
        </div>
        ...
        <div class="card-body mt-3">
          <div class="grid grid-cols-2">
            <div>
              <h2 class="mb-3">
                {{ $t('app.checkout.create.subscriptions.title') }}
              </h2>
            </div>
            <div class="text-right">
              <button class="btn--main" @click="addSubscriptionPlan">
                {{ $t('app.checkout.create.subscriptions.add_subscription') }}
              </button>
            </div>
          </div>
            ...
            <thead>
              <tr>
                <th>
                  {{
                    $t(
                      'app.checkout.create.subscriptions.list.subscription_plan'
                    )
                  }}
                </th>
                <th>
                  {{ $t('app.checkout.create.subscriptions.list.per_seat') }}
                </th>
                <th>
                  {{ $t('app.checkout.create.subscriptions.list.price') }}
                </th>
                <th></th>
              </tr>
            </thead>
            <tbody v-if="checkout.subscription_plans.length === 0">
              <tr>
                <td colspan="3" class="text-center">
                  {{ $t('app.quotes.create.subscriptions.no_subscriptions') }}
                </td>
              </tr>
            </tbody>
            <tbody v-else>
              <tr v-for="(plan, key) in checkout.subscription_plans">
                <td>
                  <select class="form-field" v-model="plan.plan">
                    <option
                      v-for="subscriptionPlan in this.plans"
                ...
                <td>
                  <input
                    ...
                    class="form-field"
                    :disabled="
                      plan.plan.per_seat === undefined ||
                      plan.plan.per_seat !== true
                    "
                    v-model="plan.seat_number"
                  />
                </td>
                <td>
                  <select class="form-field" v-model="plan.price">
                    <option
                      v-for="price in getPrices(plan.plan.prices)"
                      :value="price"
                    >
                      {{ displayCurrency(price.amount) }}/{{ price.schedule }}
                    </option>
                  </select>
                </td>
                <td>
                  <button class="btn--danger" @click="deleteSubscription(key)">
        ...
        <div class="card-body mt-3">
          <div class="grid grid-cols-2">
            <div>
              <h2 class="mb-3">{{ $t('app.checkout.create.items.title') }}</h2>
            </div>
            <div class="text-right">
              <button class="btn--main" @click="addItem">
                {{ $t('app.checkout.create.items.add_item') }}
              </button>
            </div>
          </div>
            ...
            <tbody v-if="checkout.items.length === 0">
              <tr>
                <td colspan="5" class="text-center">
                  {{ $t('app.quotes.create.items.no_items') }}
                </td>
              </tr>
            </tbody>
            <tbody v-else>
              <tr v-for="(item, key) in checkout.items">
                <td>
                  <p
                    ...
                    v-if="
                      ...
                      errors.items[key] !== undefined &&
                      errors.items[key].description !== undefined
                    "
                  >
                    {{ errors.items[key].description }}
                  </p>
                  <input
                    type="text"
                    class="form-field"
                ...
                <td>
                  <p
                    ...
                    v-if="
                      errors.items != undefined &&
                      errors.items[key] !== undefined &&
                      errors.items[key].amount !== undefined
                    "
                  >
                    {{ errors.items[key].amount }}
                  </p>
                  <CurrencyInput v-model:value="item.amount" />
                </td>
                <td>
                  <input
                    type="checkbox"
                    class="form-field"
                    v-model="item.tax_included"
                  />
                </td>
                <td>
                  <p
                    ...
                    v-if="
                      errors.items != undefined &&
                      errors.items[key] !== undefined &&
                      errors.items[key].taxType != undefined
                    "
                  >
                    {{ errors.items[key].taxType }}
                  </p>
                  <select class="form-field" id="name" v-model="item.tax_type">
                    <option
                      v-bind:value="tax_type.id"
                ...
                <td>
                  <button class="btn--danger" @click="deleteItem(key)">
                    <i class="fa-solid fa-trash"></i>
                  </button>
                </td>
              </tr>
        ...
        <div class="mt-5">
          <SubmitButton
            :in-progress="send_checkout"
            class="btn--main"
            @click="createInvoice('checkout')"
            >{{ $t('app.checkout.create.create_quote') }}</SubmitButton
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    createCustomer: function () {
      ...
      axios
        .post('/app/customer', {
          email: this.create_customer_email,
    ...
    createInvoice: function (type = 'invoices') {
      ...
      if (
        this.checkout.subscription_plans.length === 0 &&
        this.checkout.items.length === 0
      ) {
        this.errors.main_error = this.$t(
          'app.checkout.create.errors.nothing_to_invoice'
        )
      }
      ...
      if (!sameSchedule || !sameCurrency) {
        this.errors.main_error = this.$t(
          'app.checkout.create.errors.same_currency_and_schedule'
        )
      }
      ...
      for (let key in this.checkout.items) {
        ...
        if (!item.description) {
          hasErrors = true
          errors[key].description = this.$t(
            'app.checkout.create.errors.need_description'
          )
        }
        ...
        if (!item.tax_type) {
          hasErrors = true
          errors[key].taxType = this.$t(
            'app.checkout.create.errors.need_tax_type'
          )
        }
        items.push({
      ...
      axios
        .post('/app/checkout/create', payload)
        .then((response) => {
          this.send_checkout = false
          this.success = true
          this.$router.push({
