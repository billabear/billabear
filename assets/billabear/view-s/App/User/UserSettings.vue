  ...
  <div class="p-5">
    <LoadingScreen :ready="loading" :loading-message="$t('global.loading')">
      ...
      <form @submit.prevent="save">
        <div class="card-body">
          <label
            ...
            >{{ $t('app.user.settings.name') }}</label
          >
          <input
            type="text"
            class="form-field-input"
            :class="{ 'form-error': errors.name !== undefined }"
            v-model="user.name"
          />
          <template v-for="error in errors.name">
            <span v-if="errors.name" class="error-message">{{ error }}</span>
          </template>

          <label
            class="block mt-5 mb-2 text-sm font-medium text-gray-900 dark:text-white"
            >{{ $t('app.user.settings.email') }}</label
          >
          <input
            type="text"
            class="form-field-input"
            :class="{ 'form-error': errors.email !== undefined }"
            v-model="user.email"
          />
          <template v-for="error in errors.email">
            <span v-if="errors.email" class="error-message">{{ error }}</span>
          </template>

          <label
            class="block mt-5 mb-2 text-sm font-medium text-gray-900 dark:text-white"
            >{{ $t('app.user.settings.locale') }}</label
          >
          ...
          <template v-for="error in errors.locale">
            <span v-if="errors.locale" class="error-message">{{ error }}</span>
          </template>
        </div>

        <div class="mt-3">
          <SubmitButton
            :in-progress="sending_settings"
            :loading-text="$t('app.user.settings.in_progress')"
            >{{ $t('app.user.settings.save') }}</SubmitButton
          >
        </div>
      </form>

      <h2
        class="h2 text-slate-500 mt-3 cursor-pointer"
        @click="show_danger_zone = !show_danger_zone"
      >
        {{ $t('app.user.settings.danger_zone') }}
        <i class="fa-solid fa-caret-down" v-if="!show_danger_zone"></i
      ...
      <form @submit.prevent="changePassword" v-if="show_danger_zone">
        <div class="card-body">
          <label class="label">{{
            $t('app.user.settings.current_password')
          }}</label>
          <input
            type="password"
            class="form-field-input"
          ...
          />
          <span class="error-message" v-if="need_current_password">{{
            $t('app.user.settings.need_current_password')
          }}</span>

          <label class="label">{{
            $t('app.user.settings.new_password')
          }}</label>
          <input
            type="password"
            name="password"
          ...
          />
          <span class="error-message" v-if="need_new_password">{{
            $t('app.user.settings.need_new_password')
          }}</span>
          <span class="error-message" v-if="need_valid_password">{{
            $t('app.user.settings.need_valid_password')
          }}</span>

          <label class="label">{{
            $t('app.user.settings.new_password_again')
          }}</label>
          <input
            type="password"
            class="form-field-input"
          ...
          />
          <span class="error-message" v-if="need_passwords_to_match">{{
            $t('app.user.settings.need_password_to_match')
          }}</span>
        </div>
        <div class="mt-3">
          <SubmitButton
            :in-progress="sending_password"
            :loading-text="$t('app.user.settings.in_progress')"
          >
...
export default {
  ...
  mounted() {
    userservice.fetchSettings().then((user) => {
      this.user = user
      this.loading = true
    })
  },
