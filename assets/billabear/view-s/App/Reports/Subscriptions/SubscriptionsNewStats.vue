<template>
  <div>
    <h1 class="page-title mb-5">
      {{ $t('app.reports.subscriptions.new_stats.title') }}
    </h1>
      ...
      <div class="grid grid-cols-3 md:grid-cols-6 gap-3 mt-5">
        <div class="stat">
          <h2 class="stat-header">
            {{ $t('app.reports.subscriptions.new_stats.totals.existing') }}
          </h2>
        ...
        <div class="stat">
          <h2 class="stat-header">
            {{ $t('app.reports.subscriptions.new_stats.totals.new') }}
          </h2>
          <div class="stat-body text-3xl">
            {{ totals.new }}
          </div>
        ...
        <div class="stat">
          <h2 class="stat-header">
            {{ $t('app.reports.subscriptions.new_stats.totals.upgrades') }}
          </h2>
          <div class="stat-body text-3xl">
            {{ totals.upgrades }}
          </div>
        ...
        <div class="stat">
          <h2 class="stat-header">
            {{ $t('app.reports.subscriptions.new_stats.totals.downgrades') }}
          </h2>
          <div class="stat-body text-3xl">
            {{ totals.downgrades }}
          </div>
        ...
        <div class="stat">
          <h2 class="stat-header">
            {{ $t('app.reports.subscriptions.new_stats.totals.cancellations') }}
          </h2>
          <div class="stat-body text-3xl">
        ...
        <div class="stat">
          <h2 class="stat-header">
            {{ $t('app.reports.subscriptions.new_stats.totals.reactivations') }}
          </h2>
          <div class="stat-body text-3xl">
...
export default {
  ...
  methods: {
    fetchData() {
      axios
        .get('/app/reports/subscriptions/new')
        .then((response) => {
    ...
    setChartData() {
      ...
      const series = [
        ...
        {
          name: this.$t(
            'app.reports.subscriptions.new_stats.labels.downgrades'
          ),
          data: filteredMonths.map((month) => month.downgrades),
        },
        {
          name: this.$t(
            'app.reports.subscriptions.new_stats.labels.cancellations'
          ),
          data: filteredMonths.map((month) => month.cancellations),
        },
        {
          name: this.$t(
            'app.reports.subscriptions.new_stats.labels.reactivations'
          ),
    ...
    calculateTotals() {
      ...
      this.totals = {
        ...
        downgrades: last12Months.reduce(
          (sum, month) => sum + month.downgrades,
          0
        ),
        cancellations: last12Months.reduce(
          (sum, month) => sum + month.cancellations,
          0
        ),
        reactivations: last12Months.reduce(
          (sum, month) => sum + month.reactivations,
          0
        ),
      }
    },
