<template>
  <div>
    <h1 class="page-title mb-5">
      {{ $t('app.reports.subscriptions.churn.title') }}
    </h1>
    <LoadingScreen :ready="ready">
      <div class="text-end my-5">
        <div class="bg-white rounded-3xl inline p-3">
          <div
            class="chart-button inline p-3 rounded-3xl"
            @click="setChartData('daily')"
            :class="{ 'chart-button-selected': viewName === 'daily' }"
          >
            {{ $t('app.reports.subscriptions.churn.buttons.daily') }}
          </div>
          <div
            class="chart-button inline p-3 rounded-3xl"
            @click="setChartData('monthly')"
            :class="{ 'chart-button-selected': viewName === 'monthly' }"
          >
            {{ $t('app.reports.subscriptions.churn.buttons.monthly') }}
          </div>
          <div
            class="chart-button inline p-3 rounded-3xl"
            @click="setChartData('yearly')"
            :class="{ 'chart-button-selected': viewName === 'yearly' }"
          >
      ...
      <div class="card-body">
        <div class="section-body">
          <apexchart
            ...
            height="500"
            type="bar"
            :options="overview.chartOptions"
            :series="overview.series"
          ></apexchart>
          <apexchart
            v-if="viewName == 'monthly'"
            height="500"
            type="bar"
            :options="overview.chartOptions"
            :series="overview.series"
  ...
  methods: {
    ...
    convertStatToChartData: function (input) {
      ...
      for (let i = 0; input.length > i; i++) {
        ...
        categories.push(
          data.dayDate + '-' + data.monthDate + '-' + data.yearDate
        )
