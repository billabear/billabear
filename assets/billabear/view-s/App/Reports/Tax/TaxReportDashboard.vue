  ...
  <div class="overflow-hidden">
    ...
    <LoadingScreen :ready="ready">
      <div class="mt-5 grid grid-cols-4 gap-5">
        ...
        <div class="country-body">
          <h2 class="report-subtitle">
            {{ $t('app.reports.tax.countries.title') }}
          </h2>

          <div
            class="my-3 border p-2 hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800"
            v-for="country in rawCountryData"
          >
            <div class="country-title">{{ country.country.name }}</div>
            <div
              class="country-data"
              v-html="
                $t('app.reports.tax.countries.transacted_amount', {
                  transacted_amount: displayCurrency(country.transacted_amount),
                  currency: Number()
            ...
            <div
              class="country-data"
              v-html="
                $t('app.reports.tax.countries.collected_amount', {
                  collected_amount: displayCurrency(country.collected_amount),
                  currency: Number()
                    .toLocaleString(undefined, {
                      style: 'currency',
                      currency: country.country.currency,
            ...
            <div
              class="country-data"
              v-html="
                $t('app.reports.tax.countries.threshold_status', {
            ...
            <ProgressBar
              :current="country.transacted_amount"
              :total="country.threshold_amount"
            />
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2">
        <div>
          <h3 class="text-2xl font-bold my-5 dark:text-white">
            {{ $t('app.reports.tax.transactions.title') }}
          </h3>
        </div>
        <div class="text-end my-5">
          <button @click="processExport" class="btn--main">
            <i class="fa-solid fa-download"></i>
            {{ $t('app.reports.tax.transactions.download') }}
          </button>
        </div>
      </div>
      <div
        class="overflow-hidden rounded-xl border border-gray-300 bg-white p-5 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
      >
...
export default {
  ...
  mounted() {
    axios.get('/app/tax/report').then((response) => {
      ...
      this.mapData = this.rawCountryData
        .map((obj) => {
          return {
            code: obj.country.iso_code_3,
            value: obj.collected_amount,
            formatted_value: this.displayCurrency(obj.collected_amount),
            label: obj.country.currency,
          }
        })
        .filter((obj) => {
          return obj.value > 0
  ...
  methods: {
    ...
    processExport: function () {
      ...
      axios
        .get('/app/tax/report/export', { responseType: 'blob' })
        .then((response) => {
