    ...
    <div class="grid grid-cols-2 gap-5">
      <div class="card-body">
        <div class="form-field-ctn">
          ...
          <p class="form-field-error" v-if="errors.country != undefined">
            {{ errors.country }}
          </p>
          <CountrySelect v-model:value="filters.country" />
          <p class="form-field-help">
            {{ $t('app.reports.financial.lifetime.help_info.country') }}
          </p>
        </div>
        ...
        <div class="form-field-ctn">
          ...
          <p
            ...
            v-if="errors.payment_schedule != undefined"
          >
          ...
          <select v-model="filters.payment_schedule" class="form-field">
            ...
            <option value="week">
              {{ $t('app.reports.financial.lifetime.schedules.week') }}
            </option>
            <option value="month">
              {{ $t('app.reports.financial.lifetime.schedules.month') }}
            </option>
            <option value="year">
              {{ $t('app.reports.financial.lifetime.schedules.year') }}
            </option>
          </select>
          <p class="form-field-help">
            {{
              $t('app.reports.financial.lifetime.help_info.payment_schedule')
            }}
          </p>
        </div>
        ...
        <div class="form-field-ctn">
          ...
          <p
            class="form-field-error"
            v-if="errors.subscription_plan != undefined"
          >
            {{ errors.subscription_plan }}
          </p>
          <select v-model="filters.subscription_plan" class="form-field">
            <option :value="null"></option>
            <option v-for="plan in stats.plans" :value="plan.id">
              {{ plan.name }}
            </option>
          </select>
          <p class="form-field-help">
            {{
              $t('app.reports.financial.lifetime.help_info.subscription_plan')
            }}
          </p>
        </div>
        ...
        <div class="form-field-ctn">
          ...
          <select v-model="filters.brand" class="form-field">
            <option :value="null"></option>
            <option v-for="brand in stats.brands" :value="brand.id">
              {{ brand.name }}
            </option>
          </select>
          <p class="form-field-help">
            {{ $t('app.reports.financial.lifetime.help_info.brand') }}
          </p>
        </div>

        <div class="mt-5">
          <SubmitButton :in-progress="!ready" @click="sendFilters">{{
            $t('app.reports.financial.lifetime.submit')
          }}</SubmitButton>
        </div>
      </div>
        ...
        <LoadingScreen :ready="ready">
          <div class="card-body">
            <dl class="detail-list">
              <div>
                <dt>
                  {{ $t('app.reports.financial.lifetime.customer_count') }}
                </dt>
              ...
              <div>
                ...
                <dd>
                  {{
                    $t('app.reports.financial.lifetime.lifespan_value', {
        ...
        yaxis: [
          {
            title: {
              text: this.$t(
                'app.reports.financial.lifetime.chart.lifetime_values'
              ),
            },
          },
          {
            ...
            title: {
              text: this.$t(
                'app.reports.financial.lifetime.chart.customer_counts'
              ),
            },
          },
        ],
