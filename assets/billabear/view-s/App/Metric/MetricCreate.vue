    ...
    <div class="card-body">
      <div class="form-field-ctn">
        ...
        <p class="form-field-help">
          {{ $t('app.metric.create.help_info.name') }}
        </p>
      </div>
      ...
      <div class="form-field-ctn">
        ...
        <p class="form-field-help">
          {{ $t('app.metric.create.help_info.code') }}
        </p>
      </div>
      ...
      <div class="form-field-ctn">
        ...
        <p
          ...
          v-if="errors.aggregationMethod != undefined"
        >
        ...
        <select class="form-field" v-model="metric.aggregation_method">
          <option value="count">
            {{ $t('app.metric.create.aggregation_methods.count') }}
          </option>
          <option value="sum">
            {{ $t('app.metric.create.aggregation_methods.sum') }}
          </option>
          <option value="latest">
            {{ $t('app.metric.create.aggregation_methods.latest') }}
          </option>
          <option value="unique_count">
            {{ $t('app.metric.create.aggregation_methods.unique_count') }}
          </option>
          <option value="max">
            {{ $t('app.metric.create.aggregation_methods.max') }}
          </option>
        </select>
        <p class="form-field-help">
          {{ $t('app.metric.create.help_info.aggregation_method') }}
        </p>
      </div>
      ...
      <div class="form-field-ctn" v-if="showAggregationProperty">
        ...
        <input
          type="text"
          class="form-field"
          v-model="metric.aggregation_property"
        />
        <p class="form-field-help">
          {{ $t('app.metric.create.help_info.aggregation_property') }}
        </p>
      </div>
        ...
        <table class="w-1/2">
          ...
          <tbody>
            <tr v-for="(filter, key) in metric.filters">
              <td>
                <input type="text" class="form-field" v-model="filter.name" />
              </td>
              <td>
                <input type="text" class="form-field" v-model="filter.value" />
              </td>
              <td>
                <select class="form-field" v-model="filter.type">
                  <option value="inclusive">
                    {{ $t('app.metric.create.filter_type.inclusive') }}
                  </option>
                  <option value="exclusive">
                    {{ $t('app.metric.create.filter_type.exclusive') }}
                  </option>
                </select>
              </td>
              <td>
                <button class="btn--danger" @click="removeFilter(key)">
            ...
            <tr v-if="metric.filters.length === 0">
              <td colspan="4" class="text-center">
                {{ $t('app.metric.create.filter.no_filters') }}
              </td>
            </tr>
          </tbody>
        </table>

        <button class="btn--main mt-2" @click="addFilter">
          <i class="fa-solid fa-plus"></i>
        </button>
        <p class="form-field-help">
          {{ $t('app.metric.create.help_info.filters') }}
        </p>
      </div>
    </div>
    <div class="mt-3">
      <SubmitButton @click="send" :in-progress="sending">{{
        $t('app.metric.create.create_button')
      }}</SubmitButton>
    </div>
  </div>
</template>
...
const showAggregationProperty = computed(() => {
  return !(
    metric.aggregation_method === 'count' || metric.aggregation_method === 'sum'
  )
})
