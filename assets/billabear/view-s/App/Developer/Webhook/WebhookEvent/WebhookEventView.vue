<template>
  <div>
    <h1 class="ml-5 mt-5 page-title">
      {{ $t('app.system.webhooks.event.view.title') }}
    </h1>
      ...
      <div class="p-5">
        <div class="card-body">
          <h2 class="section-header">
            {{ $t('app.system.webhooks.event.view.main.title') }}
          </h2>
          <div class="section-body">
            <dl class="detail-list">
              ...
              <div>
                <dt>
                  {{ $t('app.system.webhooks.event.view.main.created_at') }}
                </dt>
            ...
            <table class="list-table">
              <thead>
                <tr>
                  <th>
                    {{
                      $t('app.system.webhooks.event.view.responses.list.url')
                    }}
                  </th>
                  <th>
                    {{
                      $t(
                        'app.system.webhooks.event.view.responses.list.created_at'
                      )
                    }}
                  </th>
                  <th></th>
              ...
              <tbody>
                <tr v-for="response in responses">
                  ...
                  <td>
                    <button @click="showResponse(response)" class="btn--main">
                      {{
                        $t('app.system.webhooks.event.view.responses.list.view')
                      }}
                    </button>
                  </td>
                </tr>
    ...
    <VueFinalModal
      v-model:value="viewResponse.modelValue"
...
export default {
  ...
  mounted() {
    ...
    axios
      .get('/app/developer/webhook/event/' + id + '/view')
      .then((response) => {
      ...
      .catch((error) => {
        ...
        this.errorMessage = this.$t(
          'app.system.webhooks.event.view.error_message'
        )
      })
  },
  methods: {
