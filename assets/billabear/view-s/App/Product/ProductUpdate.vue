      ...
      <div class="p-5">
        <div class="card-body">
          <div class="form-field-ctn">
            ...
            <p class="form-field-help">
              {{ $t('app.product.update.help_info.name') }}
            </p>
          </div>
          <div class="form-field-ctn">
            ...
            />
            <p class="form-field-help">
              {{ $t('app.product.update.help_info.tax_rate') }}
            </p>
          </div>
          <div class="form-field-ctn">
            ...
            <select class="form-field" id="name" v-model="product.tax_type">
              <option v-for="tax_type in tax_types" :value="tax_type.id">
                {{ tax_type.name }}
              </option>
            </select>
            <p class="form-field-help">
        ...
        <div class="form-field-ctn mt-5">
          <p @click="showAdvance = !showAdvance" class="cursor-pointer">
            ...
            <span class="ml-2">{{
              $t('app.product.update.show_advanced')
            }}</span>
          </p>
        </div>
        <div class="card-body mt-5" v-if="showAdvance">
          <div class="form-field-ctn">
            ...
            />
            <p class="form-field-help">
              {{ $t('app.product.update.help_info.external_reference') }}
            </p>
          </div>
        </div>
        ...
        <div class="form-field-submit-ctn mt-5">
          <SubmitButton :in-progress="sendingInProgress">{{
            $t('app.product.update.submit_btn')
          }}</SubmitButton>
        </div>
        <p class="text-green-500 font-weight-bold" v-if="success">
          {{ $t('app.product.update.success_message') }}
        </p>
      </div>
    </form>
  </div>
...
export default {
  ...
  mounted() {
    ...
    axios
      .get('/app/product/' + productId + '/update')
      .then((response) => {
  ...
  methods: {
    send: function () {
      ...
      const productId = this.$route.params.id
      axios
        .post('/app/product/' + productId, this.product)
        .then((response) => {
