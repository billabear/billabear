    ...
    <LoadingScreen :ready="loaded">
        ...
        <div class="card-body">
          <div class="form-field-ctn">
            ...
            <p class="form-field-help">
              {{ $t('app.tax_type.update.tax_type.help_info.name') }}
            </p>
          </div>
          ...
          <div class="form-field-ctn">
            ...
            <p class="form-field-error" v-if="errors.vatSenseType != undefined">
              {{ errors.vatSenseType }}
            </p>
            ...
            <p class="form-field-help">
              {{ $t('app.tax_type.update.tax_type.help_info.vat_sense_type') }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mt-3">
        <SubmitButton :in-progress="sending" @click="send">{{
          $t('app.tax_type.update.update_button')
        }}</SubmitButton>
      </div>
    </LoadingScreen>
  </div>
...
export default {
  ...
  methods: {
    send: function () {
      ...
      const taxTypeId = this.$route.params.id
      axios
        .post('/app/tax/type/' + taxTypeId + '/update', this.type)
        .then((response) => {
          this.sending = false
        })
        .catch((error) => {
