      ...
      <div class="p-5">
          ...
          <dl class="detail-list">
            ...
            <template v-for="amount in amounts">
              <div v-if="voucher.type === 'fixed_credit'">
                <dt>
                  {{
                    $t('app.vouchers.view.main.amount', {
        ...
        <RoleOnlyView role="ROLE_ACCOUNT_MANAGER">
          <div class="text-end mt-5">
            <SubmitButton
              :in-progress="disableProgess"
              class="btn--danger"
              v-if="!voucher.disabled"
              @click="disable"
              >{{ $t('app.vouchers.view.disable') }}</SubmitButton
            >
            <SubmitButton
              :in-progress="enableProgress"
              class="btn--main"
              v-else
              @click="enable"
              >{{ $t('app.vouchers.view.enable') }}</SubmitButton
            >
          </div>
        </RoleOnlyView>
      </div>
