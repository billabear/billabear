          ...
          <div class="card-body">
            <div class="form-field-ctn">
              ...
              <p class="form-field-help">
                {{ $t('app.vouchers.create.help_info.name') }}
              </p>
            </div>
            ...
            <div class="form-field-ctn">
              ...
              <select v-model="voucher.type" class="form-field">
                <option value="percentage">
                  {{ $t('app.vouchers.create.fields.type_percentage') }}
                </option>
                <option value="fixed_credit">
                  {{ $t('app.vouchers.create.fields.type_fixed_credit') }}
                </option>
              </select>
              <p class="form-field-help">
                {{ $t('app.vouchers.create.help_info.type') }}
              </p>
            </div>
            ...
            <div v-if="voucher.type === 'percentage'">
              <div class="form-field-ctn">
                ...
                <p
                  ...
                  v-if="errors.percentage != undefined"
                >
                  {{ errors.percentage }}
                </p>
                <input
                  ...
                  class="form-field-input"
                  id="percentage"
                  v-model="voucher.percentage"
                />
                <p class="form-field-help">
                  {{ $t('app.vouchers.create.help_info.percentage') }}
                </p>
              </div>
            </div>
            <div v-if="voucher.type === 'fixed_credit'">
              <div class="form-field-ctn" v-for="(currency, key) in currencies">
                <label class="form-field-lbl" for="percentage">
                  {{
                    $t('app.vouchers.create.fields.amount', {
                      currency: currency,
                    })
                  }}
                </label>
                <p
                  class="form-field-error"
                  v-if="errors['amounts[' + key + '].amount'] != undefined"
                >
                  {{ errors['amounts[' + key + '].amount'] }}
                </p>
                <CurrencyInput v-model:value="voucher.amounts[key].amount" />
                <p class="form-field-help">
                  {{
                    $t('app.vouchers.create.help_info.amount', {
                      currency: currency,
                    })
                  }}
                </p>
            ...
            <div class="form-field-ctn">
              ...
              <p class="form-field-error" v-if="errors.entryType != undefined">
                {{ errors.entryType }}
              </p>
              <select v-model="voucher.entry_type" class="form-field">
                <option value="manual">
                  {{ $t('app.vouchers.create.fields.entry_type_manual') }}
                </option>
                <option value="automatic">
                  {{ $t('app.vouchers.create.fields.entry_type_automatic') }}
                </option>
              </select>
              <p class="form-field-help">
                {{ $t('app.vouchers.create.help_info.entry_type') }}
              </p>
            </div>
            ...
            <div class="form-field-ctn" v-if="voucher.entry_type === 'manual'">
              ...
              />
              <p class="form-field-help">
                {{ $t('app.vouchers.create.help_info.code') }}
              </p>
            </div>

            <div
              class="form-field-ctn"
              v-if="voucher.entry_type === 'automatic'"
            >
              <label class="form-field-lbl" for="entry_event">
                {{ $t('app.vouchers.create.fields.entry_event') }}
              </label>
              <p class="form-field-error" v-if="errors.entryEvent != undefined">
                {{ errors.entryEvent }}
              </p>
              <select
                class="form-field-input"
                id="entry_event"
                v-model="voucher.entry_event"
              >
                <option value="expired_card_added">
                  {{
                    $t('app.vouchers.create.fields.event_expired_card_added')
                  }}
                </option>
              </select>
              <p class="form-field-help">
                {{ $t('app.vouchers.create.help_info.entry_event') }}
              </p>
            </div>
          </div>
          <div class="mt-5">
            <SubmitButton :in-progress="inProgress">{{
              $t('app.vouchers.create.submit')
            }}</SubmitButton>
            <p class="text-green-500 font-weight-bold" v-if="success">
              {{ $t('app.vouchers.create.success_message') }}
            </p>
          </div>
        </div>
      </form>
