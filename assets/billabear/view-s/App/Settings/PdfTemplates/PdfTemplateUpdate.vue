<template>
  <div>
    <LoadingScreen :ready="ready">
      <h1 class="page-title">
        {{
          $t('app.settings.pdf_template.update.title', {
      ...
      <div class="card-body">
        <div class="form-field-ctn">
          ...
          <p class="form-field-error" v-if="errors.content != undefined">
            {{ errors.content }}
          </p>
          <textarea
            class="form-field"
          ...
          <p class="form-field-help">
            {{ $t('app.settings.pdf_template.update.help_info.template') }}
          </p>
        ...
        <div class="mt-5">
          <SubmitButton
            ...
            >{{ $t('app.settings.pdf_template.update.download') }}</SubmitButton
          >
          <SubmitButton :in-progress="sendingUpdate" @click="save">{{
            $t('app.settings.pdf_template.update.save')
          }}</SubmitButton>
        </div>
      </div>
      <VueFinalModal
...
export default {
  ...
  mounted() {
    ...
    axios
      .get('/app/settings/template/' + templateId + '/view')
      .then((response) => {
  ...
  methods: {
    ...
    save: function () {
      ...
      axios
        .post('/app/settings/template/' + templateId + '/update', {
