<template>
  <div>
    <LoadingScreen ready="ready">
      <h1 class="page-title">
        {{ $t('app.settings.pdf_template.generator_settings.title') }}
      </h1>
      ...
      <div class="m-5">
        <div class="card-body">
          <div class="form-field-ctn">
            ...
            <p class="form-field-error" v-if="errors.generator != undefined">
              {{ errors.generator }}
            </p>
            ...
            <p class="form-field-help">
              {{
                $t(
                  'app.settings.pdf_template.generator_settings.help_info.generator'
                )
              }}
            </p>
          </div>
          ...
          <div class="form-field-ctn mt-3" v-if="settings.generator == 'mpdf'">
            ...
            <p class="form-field-error" v-if="errors.tmp_dir != undefined">
              {{ errors.tmp_dir }}
            </p>
            ...
            <p class="form-field-help">
              {{
                $t(
                  'app.settings.pdf_template.generator_settings.help_info.tmp_dir'
                )
              }}
            </p>
          </div>
          ...
          <div
            ...
            v-if="settings.generator == 'wkhtmltopdf'"
          >
            <label class="form-field-lbl" for="content">
              {{ $t('app.settings.pdf_template.generator_settings.bin') }}
            </label>
            <p class="form-field-error" v-if="errors.bin != undefined">
              {{ errors.bin }}
            </p>
            <input type="text" class="form-field" v-model="settings.bin" />
            <p class="form-field-help">
              {{
                $t('app.settings.pdf_template.generator_settings.help_info.bin')
              }}
            </p>
          </div>
          ...
          <div
            class="form-field-ctn mt-3"
            v-if="settings.generator == 'docraptor'"
          >
            <label class="form-field-lbl" for="content">
              {{ $t('app.settings.pdf_template.generator_settings.api_key') }}
            </label>
            <p class="form-field-error" v-if="errors.api_key != undefined">
              {{ errors.api_key }}
            </p>
            ...
            <p class="form-field-help">
              {{
                $t(
                  'app.settings.pdf_template.generator_settings.help_info.api_key'
                )
              }}
            </p>
          </div>
      ...
      <div class="m-5">
        <SubmitButton :in-progress="sending" @click="send">{{
          $t('app.settings.pdf_template.generator_settings.submit')
        }}</SubmitButton>
...
export default {
  ...
  methods: {
    send: function () {
      ...
      this.sending = true
      axios
        .post('/app/settings/pdf-generator', this.settings)
        .then((response) => {
          this.sending = false
          this.success = true
        })
