<template>
  <div>
    <h1 class="page-title">
      {{ $t('app.settings.system_settings.update.title') }}
    </h1>
      ...
      <form @submit.prevent="save">
        <div class="card-body">
          <div class="form-field-ctn">
            ...
            <p class="form-field-error" v-if="errors.systemUrl != undefined">
              {{ errors.systemUrl }}
            </p>
            <input
              type="text"
              class="form-field"
              id="system_url"
              v-model="systemSettings.system_url"
            />
            <p class="form-field-help">
              {{
                $t('app.settings.system_settings.update.help_info.system_url')
              }}
            </p>
          </div>
          ...
          <div class="form-field-ctn">
            ...
            <p class="form-field-error" v-if="errors.timezone != undefined">
              {{ errors.timezone }}
            </p>
            <select
              class="form-field"
            ...
            <p class="form-field-help">
              {{ $t('app.settings.system_settings.update.help_info.timezone') }}
            </p>
          </div>
        </div>
        ...
        <div class="mt-3 card-body">
          <div class="form-field-ctn">
            ...
            <select
              class="form-field"
              id="timezone"
              v-model="systemSettings.invoice_number_generation"
            >
              <option value="random">
                {{
                  $t(
          ...
          <div
            ...
            v-if="systemSettings.invoice_number_generation !== 'random'"
          >
            ...
            <input
              type="number"
              class="form-field"
              v-model="systemSettings.subsequential_number"
            />
            <p class="form-field-help">
              {{
                $t(
          ...
          <div
            class="form-field-ctn"
            v-if="systemSettings.invoice_number_generation === 'format'"
          >
          ...
          <div class="form-field-ctn">
            ...
            <select
              ...
              v-model="systemSettings.default_invoice_due_time"
            >
        ...
        <div class="mt-3 form-field-submit-ctn">
          <SubmitButton :in-progress="sending">{{
            $t('app.settings.system_settings.update.submit_btn')
          }}</SubmitButton>
        </div>
      </form>
    </LoadingScreen>
...
export default {
  ...
  methods: {
    save: function () {
      ...
      this.errors = {}
      axios
        .post('/app/settings/system', this.systemSettings)
        .then((response) => {
          this.sending = false
          this.success = true
        })
