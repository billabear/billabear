  ...
  <div>
    <div class="grid grid-cols-2">
      ...
      <div class="m-5 text-end">
        <button class="btn--main" @click="options.modelValue = true">
          {{ $t('app.settings.api_keys.main.add_new_button') }}
        </button>
      </div>
    </div>
          ...
          <thead>
            <tr class="border-b border-black">
              <th class="text-left pb-2">
                {{ $t('app.settings.api_keys.main.list.name') }}
              </th>
              <th class="text-left pb-2">
                {{ $t('app.settings.api_keys.main.list.key') }}
              </th>
              <th class="text-left pb-2">
                {{ $t('app.settings.api_keys.main.list.expires_at') }}
              </th>
              <th class="text-left pb-2">
                {{ $t('app.settings.api_keys.main.list.created_at') }}
              </th>
              <th></th>
            </tr>
          </thead>
            ...
            <tr v-for="key in apiKeys">
              ...
              <td>
                <button
                  ...
                  class="btn--danger"
                >
                  {{ $t('app.settings.api_keys.main.list.disable_button') }}
                </button>
              </td>
            </tr>
            <tr v-if="apiKeys.length === 0">
              <td colspan="5" class="py-3 text-center">
                {{ $t('app.settings.api_keys.main.list.no_api_keys') }}
              </td>
            </tr>
          </tbody>
        </table>
    ...
    >
      <div>
        <h3 class="mb-4 text-2xl font-semibold">
          {{ $t('app.settings.api_keys.main.create.title') }}
        </h3>
        <div>
          <span class="block text-lg font-medium">{{
            $t('app.settings.api_keys.main.create.name')
          }}</span>
          <span class="block font-red" v-if="errors.name != undefined">{{
            errors.name
          }}</span>
        ...
        <div>
          <span class="block text-lg font-medium">{{
            $t('app.settings.api_keys.main.create.expires')
          }}</span>
          <span class="font-red block" v-if="errors.expiresAt != undefined">{{
            errors.expiresAt
          }}</span>
          <VueDatePicker
            class="mt-2"
            v-model:value="newApiKey.expires"
        ...
        <div class="mt-5 text-center">
          ...
          >
            {{ $t('app.settings.api_keys.main.create.close') }}
          </button>
          <SubmitButton
            @click="createApiKey"
            :in-progress="sendingInProgress"
            >{{
              $t('app.settings.api_keys.main.create.create_button')
            }}</SubmitButton
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    disable: function (key) {
      axios
        .post('/app/settings/api-key/' + key.id + '/disable')
        .then((response) => {
          key.active = false
        })
    },
    createApiKey: function () {
      ...
      axios
        .post('/app/settings/api-key', payload)
        .then((response) => {
