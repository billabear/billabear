<template>
  <div>
    <h1 class="page-title">
      {{ $t('app.settings.email_template.create.title') }}
    </h1>
    ...
    <form @submit.prevent="send">
      <div class="card-body">
        <div class="form-field-ctn">
          ...
          <p class="form-field-help">
            {{ $t('app.settings.email_template.create.help_info.name') }}
          </p>
        </div>
        <div class="form-field-ctn">
          ...
          <select v-model="emailTemplate.brand" class="form-field">
            <option v-for="(name, code) in brands" :value="code">
              {{ name }}
            </option>
          </select>
          <p class="form-field-help">
            {{ $t('app.settings.email_template.create.help_info.brand') }}
          </p>
        </div>
        ...
        <div class="form-field-ctn">
          ...
          <input
            ...
            class="form-field"
            id="locale"
            v-model="emailTemplate.locale"
          />
          <p class="form-field-help">
            {{ $t('app.settings.email_template.create.help_info.locale') }}
          </p>
        </div>
        ...
        <div class="form-field-ctn">
          ...
          <p
            ...
            v-if="errors.use_emsp_template != undefined"
          >
            {{ errors.use_emsp_template }}
          </p>
          <input
            type="checkbox"
            class="form-field"
            id="use_emsp_template"
            v-model="emailTemplate.use_emsp_template"
          />
          <p class="form-field-help">
            {{
              $t(
                'app.settings.email_template.create.help_info.use_emsp_template'
              )
            }}
          </p>
        </div>
        <div
          ...
          v-if="emailTemplate.use_emsp_template === false"
        >
          <label class="form-field-lbl" for="subject">
            {{ $t('app.settings.email_template.create.fields.subject') }}
          </label>
          <p class="form-field-error" v-if="errors.subject != undefined">
            {{ errors.subject }}
          </p>
          <input
            type="text"
        ...
        >
          ...
          <p class="form-field-error" v-if="errors.template_body != undefined">
          ...
          <textarea
            ...
            rows="9"
            cols="60"
            v-model="emailTemplate.template_body"
          ></textarea>
          <p class="form-field-help">
            {{
              $t('app.settings.email_template.create.help_info.template_body')
            }}
          </p>
        </div>
        <div
          class="form-field-ctn"
          v-if="emailTemplate.use_emsp_template === true"
        >
          <label class="form-field-lbl" for="template_id">
            {{ $t('app.settings.email_template.create.fields.template_id') }}
          </label>
          <p class="form-field-error" v-if="errors.template_id != undefined">
            {{ errors.template_id }}
          </p>
          <input
            type="text"
            class="form-field"
            id="template_id"
            v-model="emailTemplate.template_id"
          />
          <p class="form-field-help">
            {{ $t('app.settings.email_template.create.help_info.template_id') }}
          </p>
        </div>
        <p class="mt-4 form-field-help">
          <a
            :href="
              'https://docs.billabear.com/user/templates/email_variables?utm=' +
              origin +
              '&utm_campaign=billabear_doc_links&utm_medium=email_variables'
            "
            >{{
              $t('app.settings.email_template.update.help_info.variable_docs')
            }}</a
          >
        </p>
      </div>
      <div class="mt-3 form-field-submit-ctn">
        <SubmitButton :in-progress="sendingInProgress">{{
          $t('app.settings.email_template.create.submit_btn')
        }}</SubmitButton>
      </div>
      <p class="text-green-500 font-weight-bold" v-if="success">
        {{ $t('app.settings.email_template.create.success_message') }}
      </p>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    send: function () {
      ...
      this.errors = {}
      axios
        .post('/app/settings/email-template', this.emailTemplate)
        .then((response) => {
