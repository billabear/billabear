<template>
  <div>
    <h1 class="page-title">
      {{ $t('app.settings.email_template.update.title') }}
    </h1>
    ...
    <form @submit.prevent="send">
      <div class="card-body">
        ...
        <div class="form-field-ctn">
          ...
          <p class="form-field-help">
            {{
              $t(
                'app.settings.email_template.update.help_info.use_emsp_template'
              )
            }}
          </p>
        </div>
        ...
        >
          ...
          <p class="form-field-help">
            {{
              $t('app.settings.email_template.update.help_info.template_body')
            }}
          </p>
        </div>
        <div
        ...
        >
          ...
          />
          <p class="form-field-help">
            {{ $t('app.settings.email_template.update.help_info.template_id') }}
          </p>
        </div>
      ...
      <div class="mt-3 form-field-submit-ctn">
        <Button
          @click="sendTemplate"
          :in-progress="sendingInProgress"
          class="btn--secondary"
          >{{ $t('app.settings.email_template.update.test_email') }}</Button
        >
        <SubmitButton :in-progress="sendingInProgress" class="ml-5">{{
          $t('app.settings.email_template.update.submit_btn')
        }}</SubmitButton>
      </div>
      <p class="text-green-500 font-weight-bold" v-if="success">
        {{ $t('app.settings.email_template.update.success_message') }}
      </p>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    sendTemplate: function () {
      ...
      axios
        .post(
          '/app/settings/email-template/' + id + '/test',
          this.emailTemplate
        )
        .then((response) => {
    ...
    send: function () {
      ...
      this.errors = {}
      axios
        .post('/app/settings/email-template/' + id, this.emailTemplate)
        .then((response) => {
          this.sendingInProgress = false
          this.success = true
        })
