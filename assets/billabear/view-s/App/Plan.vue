  ...
  <div class="h-full flex flex-col">
    ...
    <div class="my-5 text-end">
      ...
      <select v-model="paymentSchedule">
        <option value="yearly">
          {{ $t('app.plan.main.payment_schedule_yearly') }}
        </option>
        <option value="monthly">
          {{ $t('app.plan.main.payment_schedule_monthly') }}
        </option>
      </select>
    </div>
    ...
    <div class="columns md:flex md:flex-row gap-4">
      <div
        ...
        :class="{ 'border-2 border-red-500': isCurrentPlan(planName) }"
      >
        <h2 class="h2">{{ plan.name }}</h2>
        <h3 class="text-xl text-red-500" v-if="isCurrentPlan(planName)">
          {{ $t('app.plan.main.your_current_plan') }}
        </h3>
        <div
          class="plan_head_rgt my-3"
          v-if="plan.prices[paymentSchedule] !== undefined"
        >
          <h4 class="h1">
            ${{ plan.prices[paymentSchedule].usd.amount
            }}<span
              >/{{
                $t('app.plan.main.payment_schedule_' + paymentSchedule)
              }}</span
            >
          </h4>
        </div>
        <div class="plans_bdy">
          ...
          <div class="media" v-for="limit in plan.limits">
            <i class="fa fa-check"></i> {{ limit.limit }}
            {{ limit.description }}
          </div>
        </div>
        <div class="button-container" v-if="in_progress === false">
          <button
            class="btn--main block w-full"
            @click="select(planName, paymentSchedule)"
            v-if="
              current_plan === null ||
              !(
                this.current_plan.status == 'active' ||
                this.current_plan.status == 'pending'
              )
            "
          >
            {{ $t('app.plan.main.select_plan') }}
          </button>
          <button
            class="btn--main block w-full"
            @click="change(planName, paymentSchedule)"
            v-else-if="
              !isCurrentPlan(planName) ||
              current_plan.payment_schedule !== paymentSchedule
            "
          >
            {{ $t('app.plan.main.change') }}
          </button>
          <button class="btn--main--disabled w-full" disabled v-else>
            {{ $t('app.plan.main.selected_plan') }}
          </button>
        </div>
    ...
    >
      ...
      <a href="/app/payments/portal" class="btn--main mb-3 text-center block">{{
        $t('app.plan.main.payment_settings')
      }}</a>

      <a
        @click="cancel"
        class="btn--danger block text-center cursor-pointer"
        v-if="in_progress === false"
        >{{ $t('app.plan.main.cancel_button') }}</a
      >
...
export default {
  ...
  mounted() {
    planservice.fetchPlanInfo().then((response) => {
      ...
      if (
        ...
        response.data.current_plan !== undefined
      ) {
  ...
  methods: {
    ...
    isCurrentPlan: function (planName) {
      ...
      return (
        planName === this.current_plan.plan_name &&
        (this.current_plan.status == 'active' ||
          this.current_plan.status == 'pending')
      )
    },
