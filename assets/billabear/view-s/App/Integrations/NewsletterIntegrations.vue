<template>
  <div>
    <h1 class="page-title ml-5 mt-5">
      {{ $t('app.integrations.newsletter.title') }}
    </h1>
    ...
    <LoadingScreen :ready="ready">
      ...
      >
        ...
        <div class="form-field-ctn">
          ...
          <span
            class="form-field-error block"
            v-if="errors.enabled != undefined"
            >{{ $t(errors.enabled) }}</span
          >
...
export default {
  ...
  methods: {
    ...
    saveSettings() {
      ...
      if (
        this.marketing_list_id === null &&
        this.announcement_list_id === null &&
        this.enabled
      ) {
        this.send_request = false
        this.errors['enabled'] =
          'app.integrations.newsletter.errors.list_required'
        return
      }
      ...
      axios
        .post('/app/integrations/newsletter/settings', payload)
        .then((response) => {
          this.handleResponse(response)
