  ...
  <div v-if="!has_error">
    <div class="grid grid-cols-2">
      <h1 class="ml-5 mt-5 page-title">
        {{ $t('app.system.integrations.slack.webhooks.list.title') }}
      </h1>

      <div class="text-end mt-5">
        <router-link
          :to="{ name: 'app.system.integrations.slack.webhook.create' }"
          class="btn--main ml-4"
          ><i class="fa-solid fa-plus"></i>
          {{
            $t('app.system.integrations.slack.webhooks.list.create_new')
          }}</router-link
        >
      </div>
    </div>
          ...
          <thead>
            <tr class="border-b border-black">
              <th class="text-left pb-2">
                {{ $t('app.system.integrations.slack.webhooks.list.name') }}
              </th>
              <th class="text-left pb-2">
                {{ $t('app.system.integrations.slack.webhooks.list.webhook') }}
              </th>
              <th></th>
            </tr>
          </thead>
          <tbody v-if="loaded">
            <tr v-for="webhook in invoices" class="mt-5">
              ...
              <td class="py-3">
                <SubmitButton
                  class="btn--danger"
                  @click="disableWebhook(webhook)"
                  :in-progress="in_progress"
                  v-if="webhook.enabled"
                  >{{
                    $t(
                      'app.system.integrations.slack.webhooks.list.disable_btn'
                    )
                  }}</SubmitButton
                >
                <SubmitButton
                  class="btn--main"
                  @click="enableWebhook(webhook)"
                  :in-progress="in_progress"
                  v-else
                  >{{
                    $t('app.system.integrations.slack.webhooks.list.enable_btn')
                  }}</SubmitButton
                >
              </td>
            </tr>
            <tr v-if="invoices.length === 0">
              <td colspan="4" class="py-3 text-center">
                {{
                  $t('app.system.integrations.slack.webhooks.list.no_webhooks')
                }}
              </td>
            </tr>
          </tbody>
          <tbody v-else>
            <tr>
              <td colspan="4" class="text-center">
                <LoadingMessage>{{
                  $t('app.system.integrations.slack.webhooks.list.loading')
                }}</LoadingMessage>
              </td>
            </tr>
          </tbody>
      ...
      <div class="sm:grid sm:grid-cols-2">
        <div class="mt-4">
          <button @click="prevPage" v-if="show_back" class="btn--main mr-3">
            {{ $t('app.system.integrations.slack.webhooks.list.prev') }}
          </button>
          <button @click="nextPage" v-if="has_more" class="btn--main">
            {{ $t('app.system.integrations.slack.webhooks.list.next') }}
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    disableWebhook: function (webhook) {
      ...
      axios
        .post('/app/integrations/slack/webhook/' + webhook.id + '/disable')
        .then((response) => {
    ...
    enableWebhook: function (webhook) {
      this.in_progress = true
      axios
        .post('/app/integrations/slack/webhook/' + webhook.id + '/enable')
        .then((response) => {
          webhook.enabled = response.data.enabled
          this.in_progress = false
        })
