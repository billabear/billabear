<template>
  <div>
    <h1 class="page-title">
      {{ $t('app.workflows.payment_creation.edit.title') }}
    </h1>
  </div>
  ...
  >
    <h3>
      {{ $t('app.workflows.payment_creation.edit.add_place_modal.title') }}
    </h3>
    ...
    <dl class="detail-list">
      <div>
        <dt>
          {{
            $t('app.workflows.payment_creation.edit.add_place_modal.from_place')
          }}
        </dt>
        <dd>
      ...
      <div>
        <dt>
          {{
            $t('app.workflows.payment_creation.edit.add_place_modal.to_place')
          }}
        </dt>
        <dd>
      ...
      <div>
        <dt>
          {{
            $t('app.workflows.cancellation_request.edit.add_place_modal.name')
          }}
        </dt>
        <dd>
      ...
      <div>
        <dt>
          {{
            $t(
              'app.workflows.payment_creation.edit.add_place_modal.event_handler'
            )
          }}
        </dt>
        <dd>
    ...
    >
      <h2>
        {{
          $t(
            'app.workflows.payment_creation.edit.add_place_modal.handler_options'
          )
        }}
      </h2>
      <dl class="detail-list">
        <div v-for="(option, key) in createModalValues.handler.options">
          ...
          <dd>
            <span
              ...
              >{{
                $t(
                  'app.workflows.payment_creation.edit.add_place_modal.required'
                )
              }}</span
            >
            <input
    ...
    <div class="mt-5">
      <button
        ...
        "
      >
        {{ $t('app.workflows.payment_creation.edit.add_place_modal.add') }}
      </button>
    </div>
  </VueFinalModal>
  ...
  >
    <h3>
      {{ $t('app.workflows.payment_creation.edit.edit_place_modal.title') }}
    </h3>
    ...
    >
      {{
        $t(
          'app.workflows.payment_creation.edit.edit_place_modal.disable_button'
        )
      }}
    </button>
    <button
    ...
    >
      {{
        $t('app.workflows.payment_creation.edit.edit_place_modal.enable_button')
      }}
    </button>
  </VueFinalModal>
</template>
...
const sync = () => {
  axios.get('/app/workflow/payment-creation/edit').then((request) => {
    ...
    for (let i = 0; i < placesRawData.length; i++) {
      ...
      if (prevElement !== null) {
        edgesElement.push({
          ...
          label: t('app.workflows.payment_creation.edit.add_place'),
          animated: false,
        })
      }
