<template>
  <div>
    <h1 class="page-title">
      {{ $t('app.workflows.refund_created_process.edit.title') }}
    </h1>
  </div>
  <LoadingScreen :ready="ready">
    <div style="height: 1000px; width: 500px">
      <VueFlow
        :nodes="flowchartElements"
  ...
  >
    <h3>
      {{
        $t('app.workflows.refund_created_process.edit.add_place_modal.title')
      }}
    </h3>
    ...
    <dl class="detail-list">
      <div>
        <dt>
          {{
            $t(
              'app.workflows.refund_created_process.edit.add_place_modal.from_place'
            )
          }}
        </dt>
        <dd>
      ...
      <div>
        <dt>
          {{
            $t(
              'app.workflows.refund_created_process.edit.add_place_modal.to_place'
            )
          }}
        </dt>
        <dd>
      ...
      <div>
        <dt>
          {{
            $t('app.workflows.refund_created_process.edit.add_place_modal.name')
          }}
        </dt>
        <dd>
      ...
      <div>
        <dt>
          {{
            $t(
              'app.workflows.refund_created_process.edit.add_place_modal.event_handler'
            )
          }}
        </dt>
        <dd>
          <select v-model="createModalValues.handler">
            <option v-for="handler in handlersRawData" :value="handler">
              {{ handler.name }}
            </option>
          </select>
        </dd>
    ...
    <div
      class="mt-5"
      v-if="
        createModalValues.handler !== null &&
        createModalValues.handler !== undefined
      "
    >
      <h2>
        {{
          $t(
            'app.workflows.refund_created_process.edit.add_place_modal.handler_options'
          )
        }}
      </h2>
      <dl class="detail-list">
        <div v-for="(option, key) in createModalValues.handler.options">
          ...
          <dd>
            <span
              class="error-message"
              v-if="handlerOptionsErrors[key] !== undefined"
              >{{
                $t(
                  'app.workflows.refund_created_process.edit.add_place_modal.required'
                )
              }}</span
            >
            <input
    ...
    <div class="mt-5">
      <button
        ...
        :class="{
          'btn--disabled':
            createModalValues.name === '' || createModalValues.handler === null,
        }"
        @click="sendCreate"
        :disabled="
          createModalValues.name === '' || createModalValues.handler === null
      ...
      >
        {{
          $t('app.workflows.refund_created_process.edit.add_place_modal.add')
        }}
      </button>
    </div>
  </VueFinalModal>
  ...
  >
    <h3>
      {{
        $t('app.workflows.refund_created_process.edit.edit_place_modal.title')
      }}
    </h3>

    <button
      class="btn--danger"
      @click="sendDisable(editModalValues.node.data.id)"
      v-if="editModalValues.node.data.enabled"
    >
      {{
        $t(
          'app.workflows.refund_created_process.edit.edit_place_modal.disable_button'
        )
      }}
    </button>
    <button
      class="btn--main"
      @click="sendEnable(editModalValues.node.data.id)"
      v-else
    >
      {{
        $t(
          'app.workflows.refund_created_process.edit.edit_place_modal.enable_button'
        )
      }}
    </button>
  </VueFinalModal>
...
const { onEdgeDoubleClick, onNodeDoubleClick } = useVueFlow()
...
function sendCreate(event) {
  ...
  const priority =
    parseInt(values.source.node.id) +
    (parseInt(values.target.node.id) - parseInt(values.source.node.id)) / 2
