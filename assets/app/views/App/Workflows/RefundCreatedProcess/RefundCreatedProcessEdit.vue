<template>
  <div>
    <h1 class="page-title">{{ $t('app.workflows.refund_created_process.edit.title') }}</h1>
  </div>
  ...
  >
    <h3>{{ $t('app.workflows.refund_created_process.edit.add_place_modal.title') }}</h3>

    <dl class="detail-list">
      <div>
        <dt>{{ $t('app.workflows.refund_created_process.edit.add_place_modal.from_place') }}</dt>
        <dd>
          {{ createModalValues.source.node.label }}
        </dd>
      </div>
      <div>
        <dt>{{ $t('app.workflows.refund_created_process.edit.add_place_modal.to_place') }}</dt>
        <dd>
          {{ createModalValues.target.node.label }}
        </dd>
      </div>
      <div>
        <dt>{{ $t('app.workflows.refund_created_process.edit.add_place_modal.name') }}</dt>
        <dd>
          <input type="text" v-model="createModalValues.name" />
        </dd>
      </div>
      <div>
        <dt>{{ $t('app.workflows.refund_created_process.edit.add_place_modal.event_handler') }}</dt>
        <dd>
    ...
    <div class="mt-5" v-if="createModalValues.handler !== null && createModalValues.handler !== undefined">
      <h2>{{ $t('app.workflows.refund_created_process.edit.add_place_modal.handler_options') }}</h2>
      <dl class="detail-list">
        <div v-for="(option, key) in createModalValues.handler.options">
          <dt>{{ key }}</dt>
          <dd>
            <span class="error-message" v-if="handlerOptionsErrors[key] !== undefined">{{ $t('app.workflows.refund_created_process.edit.add_place_modal.required') }}</span>
    ...
    <div class="mt-5">
      <button class="btn--main" :class="{'btn--disabled' : (createModalValues.name === '' || createModalValues.handler === null)}" @click="sendCreate" :disabled="createModalValues.name === '' || createModalValues.handler === null">{{ $t('app.workflows.refund_created_process.edit.add_place_modal.add') }}</button>
    </div>
  </VueFinalModal>
  ...
  <VueFinalModal
    ...
    <h3>{{ $t('app.workflows.refund_created_process.edit.edit_place_modal.title') }}</h3>

    <button class="btn--danger" @click="sendDisable(editModalValues.node.data.id)" v-if="editModalValues.node.data.enabled">{{ $t('app.workflows.refund_created_process.edit.edit_place_modal.disable_button') }}</button>
    <button class="btn--main" @click="sendEnable(editModalValues.node.data.id)" v-else>{{ $t('app.workflows.refund_created_process.edit.edit_place_modal.enable_button') }}</button>
  </VueFinalModal>
</template>
...
onEdgeDoubleClick(event => {
  ...
  createModalValues = ref({
    ...
    handler: null,
    target: {
      node: event.edge.targetNode,
    },
    source: {
      node: event.edge.sourceNode,
    }
  })
})
const sync = () => {
    ...
    for (var i = 0; i < placesRawData.length; i++) {
      ...
      if (prevElement !== null) {
        edgesElement.push({
          ...
          label: t('app.workflows.refund_created_process.edit.add_place'),
          animated: false,
        })
      }
