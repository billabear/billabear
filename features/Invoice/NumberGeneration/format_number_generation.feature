Feature: Generate new invoices using a format for the number generation
  ...
  Scenario:
    ...
    And the subsequential invoice number is 13
    And the invoice number generation is format "RD%S"
    ...
    And the latest invoice for "customer.one@example.org" will have the invoice number "RD14"
  ...
  Scenario:
    ...
    And the following credit transactions exist:
      ...
      | customer.one@example.org | credit | 1100  | USD      |
    And the subsequential invoice number is 13
    And the invoice number generation is format "RD%R"
    When the background task to reinvoice active subscriptions
    Then the subscription for "customer.one@example.org" will expire in a week
    And the latest invoice for "customer.one@example.org" will not have the invoice number "RD14"
    And the latest invoice for "customer.one@example.org" will have the invoice number starting with "RD" and 8 chars
    Then the invoice subsequential number is 14
