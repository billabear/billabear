Feature: Overdue notification
  ...
  Scenario:
    Given the following invoices exist:
      | Customer                 | Paid   | Due Date |
      | customer.one@example.org | true   | -10 days |
      | customer.two@example.org | false  | -10 days |
    When the background task to send reminders for unpaid invoices
    Then the workflow for the invoice for "customer.two@example.org" will be at "customer_warning_sent"